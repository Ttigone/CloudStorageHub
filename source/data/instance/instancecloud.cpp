...
void ManCloud::getObject(const QString &jobId, const QString &bucketName,
...
void ManCloud::putObject(const QString &jobId, const QString &bucketName,
                         const QString &key, const QString &localPath) {
  auto callback = [=](qulonglong transferred_size, qulonglong total_size,
                      void *) {
    assert(transferred_size <= total_size);
...
QString ManCloud::currentBucketName() const { return m_currentBucketName; }

QString ManCloud::currentDir() const { return m_currentDir; }

void ManCloud::bucketsAlready(const QList<TtBucket> &buckets) {
