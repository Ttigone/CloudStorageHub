...
void InstanceBuckets::setBuckets() {
  ...
  // QList<TtBucket> buckets = dao.setBuckets(":/testing/buckets1.json");
  // BaseClouds
  // QList<TtBucket> buckets = dao.setBuckets(":/testing/buckets1.json");
  QList<TtBucket> buckets = dao.setBuckets(":/testing/business.json");
...
QVariant InstanceBuckets::getBucketData(int row, int column) const {
  if (!m_model || row < 0 || row >= m_model->rowCount() || column < 0 ||
      column >= m_model->columnCount()) {
    return QVariant();
  }
...
void InstanceBuckets::refreshBuckets() {
  ...
  qDebug() << "Buckets refreshed, total:"
...
bool InstanceBuckets::updateBucketName(int row, const QString &newName) {
  ...
  // 如果成功，同时更新工具提示
  if (success) {
    m_model->setData(index, QString("存储桶名称：%1").arg(newName),
                     Qt::ToolTipRole);
    emit modelChanged();
  }
